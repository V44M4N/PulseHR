...
export default function Documents() {
  const query = useModuleQuery<any>(['documents'], '/documents?page=1&limit=100');
  const documents = query.data?.data?.documents ?? [];
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <div className="card-surface p-6 lg:col-span-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            {documents.map((d: any) => (
              <div key={d.id} className="group flex items-center gap-3 p-4 rounded-xl border border-border hover:border-accent/40 hover:shadow-soft transition-all bg-card">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{d.name || d.fileName}</div>
                  <div className="text-xs text-muted-foreground flex gap-2"><span>{d.createdAt ? new Date(d.createdAt).toLocaleDateString() : '—'}</span>·<span>{d.sizeBytes ? `${Math.round(d.sizeBytes/1024)} KB` : '—'}</span></div>
                </div>
